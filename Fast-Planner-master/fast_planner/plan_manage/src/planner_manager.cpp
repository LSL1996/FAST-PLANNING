...
void FastPlannerManager::initPlanModules(ros::NodeHandle& nh) {
  ...
  if (use_geometric_path) {
    geo_path_finder_.reset(new Astar);   /// 搜索算法
    geo_path_finder_->setParam(nh);  /// 参数设置
    geo_path_finder_->setEnvironment(edt_environment_);  /// 环境设置
    geo_path_finder_->init();  /// 初始化
  }
...
bool FastPlannerManager::checkTrajCollision(double& distance) {
  ...
  while (radius < 6.0 && t_now + fut_t < local_data_.duration_) {
    ...
    double dist = edt_environment_->evaluateCoarseEDT(fut_pt, -1.0); /// evaluateCoarseEDT ：计算最近障碍物的距离
    if (dist < 0.1) {
      distance = radius;
      return false;
    }
...
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  kino_path_finder_->reset();  /// 初始化

  int status = kino_path_finder_->search(start_pt, start_vel, start_acc, end_pt, end_vel, true);   /// 搜索路径
  ...
  plan_data_.kino_path_ = kino_path_finder_->getKinoTraj(0.01);  /// 获得轨迹点
  ...
  kino_path_finder_->getSamples(ts, point_set, start_end_derivatives);  /// 获得采样点

  Eigen::MatrixXd ctrl_pts;
  NonUniformBspline::parameterizeToBspline(ts, point_set, start_end_derivatives, ctrl_pts);  /// 参数化B样条得到控制点
  ...
  ctrl_pts = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, ts, cost_function, 1, 1);  /// B样条优化轨迹得到优化后的轨迹
  ...
  while (!feasible && ros::ok()) {
    ...
    feasible = pos.reallocateTime();   ///重分配时间
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  PolynomialTraj gl_traj = minSnapTraj(pos, zero, zero, zero, zero, time);  /// 看高飞老师的课
  ...
  double            dt, duration;
  Eigen::MatrixXd   ctrl_pts = reparamLocalTraj(0.0, dt, duration);  /// 得到B样条的控制点
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {  /// simply truncate the segment and do nothing  无碰撞
    refineTraj(init_traj, time_inc);  /// 细化路径
...
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc) {
  ...
  reparamBspline(best_traj, ratio, ctrl_pts, dt, t_inc);   ///得到一系列控制点
  time_inc += t_inc;

  ctrl_pts  = bspline_optimizers_[0]->BsplineOptimizeTraj(ctrl_pts, dt, cost_function, 1, 1); ///
  best_traj = NonUniformBspline(ctrl_pts, 3, dt);  /// 优化后的控制点重新生成一条轨迹
...
void FastPlannerManager::reparamBspline(NonUniformBspline& bspline, double ratio,
  ...
  bspline.lengthenTime(ratio); ///  把轨迹给延长
  double duration = bspline.getTimeSum(); /// 获得总时间
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  int             seg_num = topo_prm_->pathLength(guide_path) / pp_.ctrl_pt_dist; /// ctrl_pt_dist:一段轨迹的长度
  Eigen::MatrixXd ctrl_pts;
  double          dt;
  ...
  guide_pt = topo_prm_->pathToGuidePts(guide_path, int(ctrl_pts.rows()) - 2);  /// 控制点转成引导点

  guide_pt.pop_back();
  guide_pt.pop_back();
  guide_pt.erase(guide_pt.begin(), guide_pt.begin() + 2);   /// 剔除首尾两点
  ...
  Eigen::MatrixXd opt_ctrl_pts1 = bspline_optimizers_[traj_id]->BsplineOptimizeTraj(
      ctrl_pts, dt, BsplineOptimizer::GUIDE_PHASE, 0, 1);     /// 给予路径点引导的轨迹优化
  ...
  plan_data_.topo_traj_pos2_[traj_id] = NonUniformBspline(opt_ctrl_pts2, 3, dt);  ///  常规优化
...
void FastPlannerManager::findCollisionRange(vector<Eigen::Vector3d>& colli_start,
  ...
  initial_traj->getTimeSpan(t_m, t_mp);   ///获得时间的间隔

  /* find range of collision */
  double t_s = -1.0, t_e;
  for (double tc = t_m; tc <= t_mp + 1e-4; tc += 0.05) {

    Eigen::Vector3d ptc = initial_traj->evaluateDeBoor(tc);  /// 计算位置
    safe = edt_environment_->evaluateCoarseEDT(ptc, -1.0) < topo_prm_->clearance_ ? false : true;    /// clearance_:该位置离障碍物的距离

    if (last_safe && !safe) {
      colli_start.push_back(initial_traj->evaluateDeBoor(tc - 0.05));  /// 将上一次未碰撞点存入也即刚开始碰撞时刻的位置
      if (t_s < 0.0)
          t_s = tc - 0.05;  /// 记录刚开始碰撞时刻
    } else if (!last_safe && safe) {
      colli_end.push_back(ptc);   ///上次不安全，本次安全，说明碰撞结束
      t_e = tc;  /// 结束碰撞时刻
    }
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  const double            forward_t = 2.0;  /// 经验值
  ...
  Eigen::Matrix3d states2pts;   ///  ????? 这个矩阵怎么来的
