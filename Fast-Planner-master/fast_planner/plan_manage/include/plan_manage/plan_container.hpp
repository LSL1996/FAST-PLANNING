...
public:
  ...
  /// 如果局部轨迹的结束时间和全局轨迹的结束时间相差小于0.1秒则认为局部轨迹到达了目标点
  bool localTrajReachTarget() { return fabs(local_end_time_ - global_duration_) < 0.1; }   /// 用时间来衡量

  ///将traj赋值给global_traj_ ，并设置多项式轨迹的段数和总时长，time赋值给global_start_time_ 为该全局轨迹的开始时间。最后清空局部轨迹。
  void setGlobalTraj(const PolynomialTraj& traj, const ros::Time& time) {
    global_traj_ = traj;
    global_traj_.init();
    global_duration_ = global_traj_.getTimeSum();  /// time_sum
  ...
  /// 将traj赋值给local_traj_[0] ，使用UniformBspline::getDerivative() 得到速度和加速度轨迹赋值给local_traj_[1]和local_traj_[2]
  void setLocalTraj(NonUniformBspline traj, double local_ts, double local_te, double time_inc) {
    local_traj_.resize(3);
    local_traj_[0] = traj;  /// pos
    local_traj_[1] = local_traj_[0].getDerivative();  /// vel
    local_traj_[2] = local_traj_[1].getDerivative();  /// acc
  ...
  /// 如果t小于局部轨迹的开始时间或大于局部轨迹的结束时间那么t时刻的位置在全局轨迹处，使用PolynomialTraj::evaluate(t) 来计算t时刻的位置/速度/加速度。
  /// 否则说明t时刻的位置在局部轨迹处，使用UniformBspline::evaluateDeBoor(t) 来计算t时刻的位置/速度/加速度。
  ...
  /// 令seg_time和seg_length为从getPosition(start_t)到距离getPosition(start_t) 长度为des_radius 的轨迹处的时长和长度。
  /// 等间隔得到轨迹上dist_pt 个点的位置，并压入point_set 。最后将start_t时刻和start_t+seg_time时刻的速度和加速度压入start_end_derivative
    ...
    while (radius < des_radius && seg_time < global_duration_ - start_t - 1e-3) {
      ...
      prev_pt = cur_pt;
      radius = (cur_pt - first_pt).norm();  ///  当前点到第一个点的距离
    }
  ...
  /// 从start_t 时刻开始以duration/seg_num 为间隔同getPosition(t) 得到各个时刻的位置并压入point_set 。
  /// 最后将start_t时刻和start_t+duration时刻的速度和加速度压入start_end_derivative 。
