...
int Astar::search(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt, bool dynamic, double time_start) {
  ...
  while (!open_set_.empty()) {
    ...
    cur_node = open_set_.top();  /// 优先级队列的函数top()
    ...
    /* ---------- expansion loop ---------- */   /// 3*3*3  每一个节点扩展了27个方向
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          ...
          NodePtr pro_node =
              dynamic ? expanded_nodes_.find(pro_id, pro_t_id) : expanded_nodes_.find(pro_id);   ///动态障碍物

          if (pro_node != NULL && pro_node->node_state == IN_CLOSE_SET) {   /// 不在闭集合
          ...
          //                         -1.0);
          double dist = edt_environment_->evaluateCoarseEDT(pro_pos, -1.0);  /// 可行性检查
          if (dist <= margin_) {
            continue;
          }
...
void Astar::setParam(ros::NodeHandle& nh) {
  ...
  tie_breaker_ = 1.0 + 1.0 / 10000;  /// 经验值
...
void Astar::retrievePath(NodePtr end_node) {
  ...
  reverse(path_nodes_.begin(), path_nodes_.end());  /// reverse
}
...
void Astar::reset() {
  ...
  for (int i = 0; i < use_node_num_; i++) {   /// 每一个节点初始化
...
Eigen::Vector3i Astar::posToIndex(Eigen::Vector3d pt) {
    /// cast:将浮点型的数据转换成int类型的，内部操作是通过round()或者floor()函数来实现的，而不是通过cast实现
