...
list<GraphNode::Ptr> TopologyPRM::createGraph(Eigen::Vector3d start, Eigen::Vector3d end) {
  ...
  /// transformation
  translation_ = 0.5 * (start + end);   /// 中间

  Eigen::Vector3d xtf, ytf, ztf, downward(0, 0, -1);
  xtf = (end - translation_).normalized();  /// normalized:归一化
  ...
  Eigen::Vector3d pt;
  ros::Time t1, t2;
  while (sample_time < max_sample_time_ && sample_num < max_sample_num_) {   /// 采样时间和数目
    t1 = ros::Time::now();
    ...
    if (visib_guards.size() == 0) {
      GraphNode::Ptr guard = GraphNode::Ptr(new GraphNode(pt, GraphNode::Guard, ++node_id));  /// 采样点不能和任何guard看得见
      graph_.push_back(guard);  /// 该点设置为guard
      ...
      graph_.push_back(connector); /// 改点设置为connector
  ...
  for (int i = 0; i < g1->neighbors_.size(); ++i) {
    for (int j = 0; j < g2->neighbors_.size(); ++j) {
      if (g1->neighbors_[i]->id_ == g2->neighbors_[j]->id_) {   ///
        ...
        bool same_topo = sameTopoPath(path1, path2, 0.0);  /// 是否是相同的一条路径
        if (same_topo) {
...
bool TopologyPRM::lineVisib(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, double thresh,
  ...
  casters_[caster_id].setInput(p1 / resolution_, p2 / resolution_);    /// 射线设置输入
  while (casters_[caster_id].step(ray_pt)) {  ///
    pt_id(0) = ray_pt(0) + offset_(0);
    pt_id(1) = ray_pt(1) + offset_(1);
    pt_id(2) = ray_pt(2) + offset_(2);
    dist = edt_environment_->sdf_map_->getDistance(pt_id);  /// 和该点最近障碍物的距离
    if (dist <= thresh) {
...
vector<vector<Eigen::Vector3d>> TopologyPRM::selectShortPaths(vector<vector<Eigen::Vector3d>>& paths,
  ...
  for (int i = 0; i < reserve_num_ && paths.size() > 0; ++i) {  ///
    ...
    if (i == 0) {
      ...
      paths.erase(paths.begin() + path_id);   /// erase()可以删除由一个iterator指出的元素，也可以删除一个指定范围的元素
    } else {
...
void TopologyPRM::shortcutPath(vector<Eigen::Vector3d> path, int path_id, int iter_num) {
  ...
  for (int k = 0; k < iter_num; ++k) {
    ...
    for (int i = 1; i < dis_path.size(); ++i) {
      if (lineVisib(short_path.back(), dis_path[i], resolution_, colli_pt, path_id))   /// 求出碰撞点
          continue;

      edt_environment_->evaluateEDTWithGrad(colli_pt, -1, dist, grad);  ///
      if (grad.norm() > 1e-3) {
        grad.normalize();
        dir = (dis_path[i] - short_path.back()).normalized();  /// 连接两点经过障碍物的线段的方向
        push_dir = grad - grad.dot(dir) * dir;   /// 垂直方向，远离障碍物方向
        push_dir.normalize();
        colli_pt = colli_pt + resolution_ * push_dir;  /// 无碰撞的点
      }
...
void TopologyPRM::shortcutPaths() {
  short_paths_.resize(raw_paths_.size());   /// raw_paths_:已经选择过后的节点最少的几条路径

  if (parallel_shortcut_) {
    vector<thread> short_threads;
    for (int i = 0; i < raw_paths_.size(); ++i) {
      short_threads.push_back(thread(&TopologyPRM::shortcutPath, this, raw_paths_[i], i, 1));   /// 多线程处理
    }
    for (int i = 0; i < raw_paths_.size(); ++i) {
...
vector<Eigen::Vector3d> TopologyPRM::discretizePath(vector<Eigen::Vector3d> path) {
  ...
  for (int i = 0; i < path.size() - 1; ++i) {
    segment = discretizeLine(path[i], path[i + 1]);  /// 离散路径点
...
vector<vector<Eigen::Vector3d>> TopologyPRM::searchPaths() {
  ...
  for (int i = 0; i < raw_paths_.size(); ++i) {
    ...
    path_list[int(raw_paths_[i].size())].push_back(i);  ///相同路径节点的路径有多条
  }
  ...
  for (int i = min_node_num; i <= max_node_num; ++i) {
    ...
    for (int j = 0; j < path_list[i].size(); ++j) {
      ...
      if (filter_raw_paths.size() >= max_raw_path2_) {  /// max_raw_path2_
        reach_max = true;
        break;
      }
    }
...
void TopologyPRM::depthFirstSearch(vector<GraphNode::Ptr>& vis) {
  ...
  for (int i = 0; i < cur->neighbors_.size(); ++i) {
    ...
    vis.pop_back();  /// pop_back()可以删除最后一个元素
  }
}
