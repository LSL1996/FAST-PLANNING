...
/// 论文2中Topological Path Searching
/* ---------- node of topo graph ---------- */
class GraphNode {
private:
  /* data */
...
private:
  ...
  /// sampling generator
  random_device rd_;  /// random_device（）目的就是产生生成真随机数，rand()函数用的线性同余法生成随机数，但是这个随机数实际上是一个伪随机数
  default_random_engine eng_; /// 随机数生成器是 std::default_random_engine 类型别名定义的随机无符号整数
  uniform_real_distribution<double> rand_pos_;   /// uniform_real_distribution 类模板定义了一个默认返回 double 型浮点值的连续分布
  ...
  int max_sample_num_;
  int max_raw_path_, max_raw_path2_;     /// max_raw_path_:深度优先搜索出的最大路径数目；max_raw_path2_:选择路径节点数目最少的路径数目
  int short_cut_num_;
  Eigen::Vector3d sample_inflate_;  ///
  double resolution_;

  double ratio_to_short_;   /// 与最短路径的比率
  int reserve_num_;  ///
  ...
  bool needConnection(GraphNode::Ptr g1, GraphNode::Ptr g2, Eigen::Vector3d pt);    /// test redundancy with existing connection between two guard
