...
/// 论文1 中的Kinodynamic Path Searching
...
class PathNode {
 public:
  ...
  Eigen::Matrix<double, 6, 1> state;  /// xyz的位置和速度
...
class KinodynamicAstar {
 private:
  ...
  bool is_shot_succ_ = false;   /// 当前位置和找到最有时间后求得的位置之间是否想通
  Eigen::MatrixXd coef_shot_;  /// 找到最有时间后求得的位置的多项式系数
  double t_shot_; /// 最优时间
  ...
  double w_time_, horizon_, lambda_heu_;  /// w_time_: 论文1gc计算公式中的rou
