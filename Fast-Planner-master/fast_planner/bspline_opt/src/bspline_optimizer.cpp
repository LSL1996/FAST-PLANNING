...
const int BsplineOptimizer::SMOOTHNESS  = (1 << 0);  /// 1 << x:1左移x位
...
const int BsplineOptimizer::GUIDE_PHASE = BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::GUIDE; /// 平滑+引号
const int BsplineOptimizer::NORMAL_PHASE =
    BsplineOptimizer::SMOOTHNESS | BsplineOptimizer::DISTANCE | BsplineOptimizer::FEASIBILITY; /// 平滑+障碍物距离+可行性
...
void BsplineOptimizer::optimize() {
  ...
  if (cost_function_ & ENDPOINT) {
    variable_num_ = dim_ * (pt_num - order_);  /// 结尾不优化
    ...
    variable_num_ = max(0, dim_ * (pt_num - 2 * order_)) ;   /// 开头结尾都不优化
  }
  ...
  for (int i = order_; i < pt_num; ++i) {
    ...
    for (int j = 0; j < dim_; j++) {
      q[dim_ * (i - order_) + j] = control_points_(i, j);   /// 要优化的变量
    }
  }
  ...
  if (dim_ != 1) {
    ...
    opt.set_upper_bounds(ub);   /// 设置上下界限
  }
  ...
  for (int i = order_; i < control_points_.rows(); ++i) {
    if (!(cost_function_ & ENDPOINT) && i >= pt_num - order_)
        continue;
    for (int j = 0; j < dim_; j++) {
      control_points_(i, j) = best_variable_[dim_ * (i - order_) + j];   /// 优化的结果存在best_variable_中
    }
  }
...
void BsplineOptimizer::calcSmoothnessCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  std::fill(gradient.begin(), gradient.end(), zero);  /// std::fill
  Eigen::Vector3d jerk, temp_j;

  for (int i = 0; i < q.size() - order_; i++) {   /// 平滑优化：0~总共的控制点-阶数
    /* evaluate jerk */
...
void BsplineOptimizer::calcDistanceCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = order_; i < end_idx; i++) {   /// 距离优化：阶数~所有控制点-阶数 或者 阶数~所有控制点
    edt_environment_->evaluateEDTWithGrad(q[i], -1.0, dist, dist_grad);  /// 求控制点的距离和梯度
...
void BsplineOptimizer::calcFeasibilityCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = 0; i < q.size() - 1; i++) {   /// 可行性：优化所有控制点
...
void BsplineOptimizer::calcEndpointCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  dq = 1 / 6.0 * (q_3 + 4 * q_2 + q_1) - end_pt_;   /// 结束点：优化最后三个点
  cost += dq.squaredNorm();
...
void BsplineOptimizer::calcWaypointsCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = 0; i < waypoints_.size(); ++i) {   /// 路径点：优化选择的一些控制点
...
void BsplineOptimizer::calcGuideCost(const vector<Eigen::Vector3d>& q, double& cost,
  ...
  for (int i = order_; i < end_idx; i++) {   /// 引导点：优化阶数~总的控制点-阶数
    Eigen::Vector3d gpt = guide_pts_[i - order_];
    cost += (q[i] - gpt).squaredNorm();   /// 一一对应
...
void BsplineOptimizer::combineCost(const std::vector<double>& x, std::vector<double>& grad,
  ...
  for (int i = 0; i < order_; i++) {
    for (int j = 0; j < dim_; ++j) {
      g_q_[i][j] = control_points_(i, j); /// 0 ~ order
    }
  ...
  for (int i = 0; i < variable_num_ / dim_; i++) {
    for (int j = 0; j < dim_; ++j) {
      g_q_[i + order_][j] = x[dim_ * i + j];  /// order ~ 优化变量数目
    }
    for (int j = dim_; j < 3; ++j) {
      g_q_[i + order_][j] = 0.0;
    }
  ...
  if (!(cost_function_ & ENDPOINT)) {   /// 前提：端点不需要优化
    for (int i = 0; i < order_; i++) {

      for (int j = 0; j < dim_; ++j) {
        g_q_[order_ + variable_num_ / dim_ + i][j] =   /// 优化变量数目 ~ order+优化变量数目
