...
// voxel hashing
template <typename T>
struct matrix_hash : std::unary_function<T, size_t> {  /// （parameter_type,result_type) 和 size_t operator()(T &t)	计算t的散列值
...
struct MappingParameters {
  ...
  Eigen::Vector3d map_min_boundary_, map_max_boundary_;  /// map range in pos   正负值
  ...
  double prob_hit_log_, prob_miss_log_, clamp_min_log_, clamp_max_log_,      /// 这几个概率分别是什么意思 ？？？
...
struct MappingData {
  ...
  vector<char> flag_traverse_, flag_rayend_;  /// flag_traverse_:体素经过的射线
  char raycast_num_;  /// 射线的编号
...
private:
  ...
  // 订阅两个及两个以上的话题时，需要保持对这两个话题数据的同步，且需要同时接收数据一起处理然后当做参数传入到另一个函数中时，
  /*
   * 方法一:利用全局变量TimeSynchronizer
   ...
   * 方法二: 利用类成员message_filters::Synchronizer
   */
  ...
  ///可以将一个随机数生成器对象作为参数值给分布函数对象来获取一个随机值。
  /// 可以通过调用对象的成员函数 pamm() 来获取和设置范围的界限。
  /// 除了返回分布界限的成员函数 min() 和 max()，uniform_real_distribution 对象也有成员函数 a() 和 b()
  /// uniform_real_distribution: 返回 double 型浮点值的连续分布
  uniform_real_distribution<double> rand_noise_;
  /// normal_distribution:正态(或高斯)分布
  normal_distribution<double> rand_noise2_;
  /// default_random_engine e1(x):创建引擎时设置种子x
...
inline double SDFMap::getDistance(const Eigen::Vector3i& id) {
  ...
  boundIndex(id1);
  return md_.distance_buffer_all_[toAddress(id1)];   /// distance_buffer_all_  ???
}
...
inline bool SDFMap::isUnknown(const Eigen::Vector3i& id) {
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
  return md_.occupancy_buffer_[toAddress(id1)] < mp_.clamp_min_log_ - 1e-3;  /// occupancy_buffer_ ???
}
...
inline bool SDFMap::isKnownFree(const Eigen::Vector3i& id) {
  ...
  return md_.occupancy_buffer_[adr] >= mp_.clamp_min_log_ && md_.occupancy_buffer_inflate_[adr] == 0;  /// occupancy_buffer_inflate_[x]==0:已知的且未占用的
}
...
inline bool SDFMap::isKnownOccupied(const Eigen::Vector3i& id) {
  ...
  return md_.occupancy_buffer_inflate_[adr] == 1;  /// 已知的且已占用的
}
...
inline double SDFMap::getDistWithGradTrilinear(Eigen::Vector3d pos, Eigen::Vector3d& grad) {
  ...
  diff = (pos - idx_pos) * mp_.resolution_inv_;   /// 位置变换前后差值
...
inline int SDFMap::getOccupancy(Eigen::Vector3d pos) {
  ...
  return md_.occupancy_buffer_[toAddress(id)] > mp_.min_occupancy_log_ ? 1 : 0;  /// occupancy_buffer_:地址被占据的概率
}
...
inline void SDFMap::posToIndex(const Eigen::Vector3d& pos, Eigen::Vector3i& id) {
  for (int i = 0; i < 3; ++i) id(i) = floor((pos(i) - mp_.map_origin_(i)) * mp_.resolution_inv_);  /// floor:位置上有偏差
}

///
inline void SDFMap::indexToPos(const Eigen::Vector3i& id, Eigen::Vector3d& pos) {
  for (int i = 0; i < 3; ++i) pos(i) = (id(i) + 0.5) * mp_.resolution_ + mp_.map_origin_(i);  /// 0.5:
}
