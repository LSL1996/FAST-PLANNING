...
private:
  ...
  double yaw_, yaw_dot_;  /// yaw_dot_：偏航角速度
...
void LinearObjModel::update(double dt) {
  ...
  pos_ = p0 + v0 * dt;
  for (int i = 0; i < 2; ++i) {
    pos_(i) = std::min(pos_(i), bound_(i));   ///  std::min/max
    pos_(i) = std::max(pos_(i), -bound_(i));
  }
  pos_(2) = std::min(pos_(2), bound_(2));
  pos_(2) = std::max(pos_(2), 0.0);  /// z:最小是0
...
bool LinearObjModel::collide(LinearObjModel& obj1, LinearObjModel& obj2) {
  ...
  if (collide) { /// 已经撞上
    ...
    for (int i = 0; i < 3; ++i) {
      if (tol[i] < tol[(i + 1) % 3] && tol[i] < tol[(i + 2) % 3]) {  ///找到碰撞超过量最小的维度
        vel1(i) = -vel1(i);
        vel2(i) = -vel2(i); /// 速度反向
