...
vector<ros::Publisher> pose_pubs;  /// obj pose (from optitrack):可以把optitrack的数据发布出来
vector<LinearObjModel> obj_models;

random_device rd;
default_random_engine eng(rd());  ///
/// uniform_real_distribution:连续均匀分布类模板
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "dynamic_obj");  /// dynamic_obj
  ros::NodeHandle node("~");
  ...
  obj_pub = node.advertise<visualization_msgs::Marker>("/dynamic/obj", 10);   /// 发布话题
  ...
  rand_acc_t = uniform_real_distribution<double>(0.0, 6.28);  /// 角度0-2π
  rand_acc_r = uniform_real_distribution<double>(_acc_r1, _acc_r2); /// 半径
  rand_acc_z = uniform_real_distribution<double>(-_acc_z, _acc_z);  /// 高度
  ...
  for (int i = 0; i < obj_num; ++i) {
    ...
    Eigen::Vector3d pos(rand_pos(eng), rand_pos(eng), rand_h(eng));  /// 随机数用法值得学习
    ...
    model.setLimits(Eigen::Vector3d(_xy_size, _xy_size, _h_size), Eigen::Vector2d(0.0, _vel),   /// 位置区间 速度区间 加速度区间
                    Eigen::Vector2d(0, 0));
...
void updateCallback(const ros::TimerEvent& e) {
  ...
  for (int i = 0; i < obj_num; ++i) {
    obj_models[i].update(dt); ///  p v yaw的更新
    visualizeObj(i);  ///
  }
  ...
  for (int i = 0; i < obj_num; ++i)
    for (int j = i + 1; j < obj_num; ++j) {
      bool collision = LinearObjModel::collide(obj_models[i], obj_models[j]);  /// 已经调了位置速度
      if (collision) {  /// 发生碰撞也要调偏航角速度
...
void visualizeObj(int id) {
  ...
  Eigen::Matrix3d rot;
  rot << cos(yaw), -sin(yaw), 0.0, sin(yaw), cos(yaw), 0.0, 0.0, 0.0, 1.0;  /// 绕着z轴旋转一定的角度
  ...
  mk.scale.x = scale(0), mk.scale.y = scale(1), mk.scale.z = scale(2);  /// 大小尺寸
  mk.color.a = 0.5, mk.color.r = color(0), mk.color.g = color(1), mk.color.b = color(2);  ///颜色

  mk.pose.orientation.w = qua.w();  /// 姿态
  mk.pose.orientation.x = qua.x();
  mk.pose.orientation.y = qua.y();
  mk.pose.orientation.z = qua.z();

  mk.pose.position.x = pos(0), mk.pose.position.y = pos(1), mk.pose.position.z = pos(2);  ///位置
