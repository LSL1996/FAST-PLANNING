...
double EDTEnvironment::distToBox(int idx, const Eigen::Vector3d& pos, const double& time) {
  ...
  Eigen::Vector3d pos_box = obj_prediction_->at(idx).evaluateConstVel(time);  /// idx:障碍物编号
...
pair<double, Eigen::Vector3d> EDTEnvironment::evaluateEDTWithGrad(const Eigen::Vector3d& pos,
  ...
  sdf_map_->getSurroundPts(pos, sur_pts, diff);  /// 获得周围八个方位的点

  double dists[2][2][2];
  getSurroundDistance(sur_pts, dists);  /// 获得周围距离

  interpolateTrilinear(dists, diff, dist, grad);  /// 获得梯度 距离
}
