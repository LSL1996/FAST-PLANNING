...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  mp_.map_origin_ = Eigen::Vector3d(-x_size / 2.0, -y_size / 2.0, mp_.ground_height_);   /// 左下角
  mp_.map_size_ = Eigen::Vector3d(x_size, y_size, z_size);

  mp_.prob_hit_log_ = logit(mp_.p_hit_);  /// logit:逻辑函数
  ...
  int buffer_size = mp_.map_voxel_num_(0) * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2);   /// x*y*z
  ...
  if (mp_.pose_type_ == POSE_STAMPED) {
    ...
    /// 将深度图和姿态同步:message_filters::Synchronizer<SyncPolicyImagePose>(SyncPolicyImagePose(100), *depth_sub_, *pose_sub_)
    sync_image_pose_.reset(new message_filters::Synchronizer<SyncPolicyImagePose>(SyncPolicyImagePose(100), *depth_sub_, *pose_sub_));
    /// 采用registerCallback()方法，注册多个回调函数,他们将按照注册顺序进行函数的回调
    /// boost::bind:绑定任意参数到某个指定值上或者将输入参数传入任意位置
   ...
   * ros::Timer ros::NodeHandle::createTimer(ros::Duration period, <callback>, bool oneshot = false);
    函数说明：period: 调用定时器回调函数时间间隔。例如，ros::Duration(0.1)，即每十分之一秒执行一次;
            回调函数: 函数，类方法，函数对象;
            oneshot: 是否只执行一次，如果已经执行过，还可以通过stop()、setPeriod(ros::Duration)和start()来规划再执行一次;
  */
...
void SDFMap::resetBuffer() {
  ...
  md_.local_bound_min_ = Eigen::Vector3i::Zero();  /// local_bound_min_:局部？？？
...
/// 看论文：Distance transform of sampled functions
...
void SDFMap::fillESDF(F_get_val f_get_val, F_set_val f_set_val, int start, int end, int dim) {
  ...
  int k = start;
  v[start] = start;
  z[start] = -std::numeric_limits<double>::max();   /// std::numeric_limits
...
int SDFMap::setCacheOccupancy(Eigen::Vector3d pos, int occ) {
  ...
  return idx_ctns;   /// 体素在的地址
}

///将接收到的深度点云转化到世界坐标系下保存到md_.proj_points_ 中
void SDFMap::projectDepthImage() {
  ...
  if (!mp_.use_depth_filter_) {  ///
    for (int v = 0; v < rows; v++) {
      row_ptr = md_.depth_image_.ptr<uint16_t>(v);  /// 注意此种表示法
      ...
      for (int u = 0; u < cols; u++) {
        ...
        proj_pt = camera_r * proj_pt + md_.camera_pos_;   /// 世界坐标系下
  ...
  /* use depth filter */
    ...
    if (!md_.has_first_depth_)
      ...
      for (int v = mp_.depth_filter_margin_; v < rows - mp_.depth_filter_margin_; v += mp_.skip_pixel_) {
        row_ptr = md_.depth_image_.ptr<uint16_t>(v) + mp_.depth_filter_margin_;  /// 地址+偏移
          ...
          if (false) {
            ...
            if (uu >= 0 && uu < cols && vv >= 0 && vv < rows) {
              if (fabs(md_.last_depth_image_.at<uint16_t>((int)vv, (int)uu) * inv_factor -
                       pt_reproj.z()) < mp_.depth_filter_tolerance_) {  /// depth_filter_tolerance_:重投影深度差值的阈值
                md_.proj_points_[md_.proj_points_cnt++] = pt_world;
              }
            } else {
              md_.proj_points_[md_.proj_points_cnt++] = pt_world;
...
/// 当该点不在地图范围内，则将该点挪到地图边界上，如果该点到相机的距离大于最远可测距离则把点挪到最远可测距离上，
/// 最后得到该点在地图中的地址；当该点在地图范围内，若该点到相机的距离大于最远可测距离，
/// 则把点挪到最远可测距离上并得到该点在地图中的地址，否则在得到该点在地图中的地址的基础上将该点为障碍物几率上升，
/// 将该点的md_.flag_rayend_设为当前调用raycastProcess()的次数
/// 更新边界，并将md_.local_updated_置为true ,最后根据概率更新栅格地图

/// raycastProcess: 知乎/CSDN
void SDFMap::raycastProcess() {
  ...
  double min_z = mp_.map_max_boundary_(2);  /// 初始化

  double max_x = mp_.map_min_boundary_(0);
  double max_y = mp_.map_min_boundary_(1);
  double max_z = mp_.map_min_boundary_(2); /// 初始化
  ...
  for (int i = 0; i < md_.proj_points_cnt; ++i) {
    ...
    if (!isInMap(pt_w)) {
      pt_w = closetPointInMap(pt_w, md_.camera_pos_);  /// 找最近的点

      length = (pt_w - md_.camera_pos_).norm();
      if (length > mp_.max_ray_length_) {  /// max_ray_length_:最大能够看到的距离
    ...
    if (vox_idx != INVALID_IDX) {
      if (md_.flag_rayend_[vox_idx] == md_.raycast_num_) {
        ...
        md_.flag_rayend_[vox_idx] = md_.raycast_num_;  /// flag_rayend_???
      }
    }
...
///去除局部规划范围外的地图，并将障碍物按无人机的大小膨胀（参数mp_.obstacles_inflation_），最后加入虚拟屋顶（参数mp_.virtual_ceil_height_），限制飞行高度。
void SDFMap::clearAndInflateLocalMap() {
  /*clear outside local*/
  const int vec_margin = 5;  /// 经验值
  ...
  int inf_step = ceil(mp_.obstacles_inflation_ / mp_.resolution_);  /// obstacles_inflation_ ???
  // int inf_step_z = 1;
  ...
  for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
    for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
      for (int z = md_.local_bound_min_(2); z <= md_.local_bound_max_(2); ++z) {
        ...
        if (md_.occupancy_buffer_[toAddress(x, y, z)] > mp_.min_occupancy_log_) {  /// min_occupancy_log_
  ...
  if (mp_.virtual_ceil_height_ > -0.5) {
    ...
    for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
      for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y) {
        md_.occupancy_buffer_inflate_[toAddress(x, y, ceil_id)] = 1;  /// 超出部分视为占用
      }
  }
}
...
/// 如果深度相机的位姿和图像更新了则需要更新栅格地图。
/// 调用projectDepthImage()将点云转换到世界坐标系下，
/// raycastProcess()根据概率更新栅格地图和clearAndInflateLocalMap()构造膨胀地图
...
///保存深度相机信息到md_.depth_image_ ，保存相机位姿到md_.camera_pos_和md_.camera_r_m_
/// 如果相机在地图范围内，则md_.occ_need_update_=true，表示需要更新栅格地图。最后令md_.flag_use_depth_fusion=true
...
void SDFMap::publishMap() {
  ...
  pcl::PointXYZ pt;
  pcl::PointCloud<pcl::PointXYZ> cloud;  /// 云是由点组成的

  Eigen::Vector3i min_cut = md_.local_bound_min_;
  Eigen::Vector3i max_cut = md_.local_bound_max_;

  int lmm = mp_.local_map_margin_ / 2;     /// local_map_margin_???   local_bound_min_???
  ...
  cloud.header.frame_id = mp_.frame_id_;
  sensor_msgs::PointCloud2 cloud_msg;
  /// toROSMsg: pcl::PointCloud<pcl::PointXYZ>转sensor_msgs::PointCloud2
  /// fromPCL:  pcl::PCLPointCloud2转sensor_msgs::PointCloud2
  pcl::toROSMsg(cloud, cloud_msg);  ///
  map_pub_.publish(cloud_msg);
}
