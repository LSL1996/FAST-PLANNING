...
/// 使用minimum snap计算出经过所有Pos点，满足始末速度、加速度和时长的多项式轨迹，一共Pos.cols()-1段多项式轨迹。
/// minimum snap可以看高飞老师的课，也可以看论文。
/// 简单来说就是求解一个 最小化加加加速度作为目标函数，满足过路标点、始末速度加速度、各段轨迹时长、轨迹连接处位置速度加速度连续的等式约束 的优化问题，并且通过一些技巧将其转化为无约束的优化问题，最终只要简单的矩阵运算就能求解出每段轨迹的多项式系数
...
PolynomialTraj fastLine4deg(Eigen::Vector3d start, Eigen::Vector3d end, double max_vel, double max_acc,
  ...
  /* first segment */
  cx = cy = cz = zero;
  double t1 = max_acc / max_jerk;  ///  加速度/加速度导数
